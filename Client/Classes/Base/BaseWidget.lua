...
-- This function will call the underlying function to set the property on the widget.
---@generic T
---@param self T | BaseWidget
---@param sPropertyName string
---@param xValue any
---@return T
function BaseWidget:SetPropertyValue(sPropertyName, xValue)
    if not self.PropertyBindings or not self.PropertyBindings[sPropertyName] then
        Console.Error("Property binding not found for property: " .. sPropertyName)
        return self
    end

    if not self.PropertyValues then
        self.PropertyValues = {}
    end

    self.PropertyValues[sPropertyName] = xValue

    return self
end

---@return any
function BaseWidget:GetPropertyValue(sPropertyName, xFallbackValue)
    if not self.PropertyValues then
        return nil
    end

    if not self.PropertyValues[sPropertyName] then
        return nil
    end

    return self.PropertyValues[sPropertyName] or xFallbackValue
end

-- Add a property binding to the widget class.
function Widget:AddPropertyBinding(sPropertyName, fnSet)
    if not self.PropertyBindings then
        self.PropertyBindings = {}
    end

    self.PropertyBindings[sPropertyName] = fnSet
end

------------------------------------------------
-- BaseWidget functions
------------------------------------------------
